...
# ----------------------------
INPUT_CHAINS_CSV = r"C:/Users/Mayank/Desktop/Earth-Mercury MGA_Tool/C_Verification_and_Optimization/2026-27_StageC_Adv_Sorted_Filtered.csv"
OUT_DIR = r"C:/Users/Mayank/Desktop/Earth-Mercury MGA_Tool/C_Verification_and_Optimization"
OUT_CSV = os.path.join(OUT_DIR, "2026-27_StageC_Results.csv")
TOPK_OUT = os.path.join(OUT_DIR, "2026-27_StageC_Top5_per_campaign.csv")
CHECKPOINT_JSON = os.path.join(OUT_DIR, "2026-27_StageC_checkpoint.json")
...
cfg = {
    ...
    "PERIAPSE_SWEEP_POINTS": 50,
    "PERIAPSE_SWEEP_RP_MAX_km": 5e5,  # absolute max rp in km
    "PERIAPSE_SWEEP_FACTOR": 0.5,  # max rp = factor * rp_required
    ...
    "STRICT_VINF_TOL_kms": 0.7,         # tolerance for strict check
    "VINF_MATCH_TOL_kms": 1.0,
    "VINF_DSM_CAP_kms": 1.5,
    ...
    "DE_POPSIZE": 30,
    "DE_MAXITER": 300,
    "DE_WORKERS": 4,  # use 1 to avoid pickling issues; set >1 with care

    "LOCAL_POLISH": True,

    # autosave
    "AUTOSAVE_EVERY": 2,  # save every N chains
    ...
    "APPLY_VINF_IN_OBJECTIVE" : True,       # If True, include vinf mismatch term in objective
    ...
    "APPLY_VINF_LOCAL_POLISH" : False,      # If True, run the local constrained vinf polish on Top-K candidates
    "VINF_LOCAL_POLISH_TOPK" : 10,           # Run the constrained polish on Top-K candidates selected from DE results
    "VINF_LOCAL_POLISH_MAXITER" : 300,      # Max iterations for local SLSQP polish
    "VINF_LOCAL_POLISH_DSM_CAP_kms" : 1.0,  # DSM limit to preserve during polish (kms)
    "VINF_LOCAL_POLISH_C3_CAP_km2s2" : 70, # Set to large if you don't want to cap C3 (or set reasonable cap)
    # ----------------------------------------------------------------------
}
